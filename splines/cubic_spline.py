...
def fitRobustCubicPolynomial(points):
    n = len(points)
    coefficients = []
    for i in range(1, n):
        first_derivative_end = 2.0/((points[i+1][0]-points[i][0])/(points[i+1][1]-points[i][1])+(points[i][0]-points[i-1][0])/(points[i][1]-points[i-1][1]))
        if(i == 1):
            first_derivative_begin = 3.0/2.0 * (points[i][1]-points[i-1][1])/(points[i][0]-points[i-1][0]) - first_derivative_end/2.0
        elif(i == n-1):
            first_derivative_begin = 3.0/2.0 * (points[i][1]-points[i-1][1])/(points[i][0]-points[i-1][0]) - first_derivative_end/2.0
        else:
            first_derivative_begin = 2.0/(points[i][0]-points[i-1][0])/(points[i][1]-points[i-1][1])+(points[i-1][0]-points[i-2][0])/(points[i-1][1]-points[i-2][1])
        d =  /(6.0*(points[i][0] - points[i-1][0]))
        coeff = np.array([a, b, c, d, points[i][0])
        coefficients.append(coeff)
    return coefficients
